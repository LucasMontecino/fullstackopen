...
const favoriteBlog = (blogs) => {
  ...
  const result = [...blogs].sort((a, b) => (a.likes > b.likes ? -1 : 1));
  return result[0];
};
...
const mostBlogs = (blogs) => {
  ...
  const blogsByAuthor = blogs.reduce((acc, blog) => {
    const existingAuthor = acc.find((item) => item.author === blog.author);
    if (existingAuthor) {
      return acc.map((item) =>
  ...
  const [mostBlogsAuthor] = [...blogsByAuthor].sort((a, b) =>
    a.blogs < b.blogs ? 1 : -1
  );
...
const mostLikes = (blogs) => {
  ...
  const [mostLikesAuthor] = [...likesByAuthor].sort((a, b) =>
    a.likes < b.likes ? 1 : -1
  );
