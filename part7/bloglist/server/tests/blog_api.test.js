...
describe('at the beginning some blogs are saved', () => {
  ...
  beforeEach(async () => {
    ...
    const passwordHash = await bcrypt.hash('lucas_2025', 10);
  ...
  describe('added new blogs', () => {
    test('a valid blog can be added', async () => {
      ...
      assert(titles.includes('Always separate app and server files!'));
    });
    ...
    test('attempted add a blog without token is forbidden', async () => {
      ...
      const blogsAtEnd = await helper.blogsInDb();
      assert.strictEqual(blogsAtEnd.length, helper.initialBlogs.length);
      assert(result.body.error.includes('you must provide a token'));
    });
  });
  ...
  describe('updating a note', () => {
    ...
    test('failed with status 404 if blog does not exist', async () => {
      const validateNonexistId = await helper.nonExistingId();

      await api.put(`/api/blogs/${validateNonexistId}`).expect(404);
    });
